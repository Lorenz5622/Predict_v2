...
"""
Launcher for running MoEAtt stage1 then stage2 with two separate torchrun invocations.

Example:
  python launch_two_stage_torchrun_moe_att.py --nproc_per_node 2 -- --config configs/moe_att_example.json
"""
...
def main() -> None:
    ...
    cmd1 = torchrun + torchrun_args + [str(repo_root / "run_stage1_moe_att.py")] + stage1_args
    _run(cmd1, env)
    ...
    cmd2 = torchrun + torchrun_args + [str(repo_root / "run_stage2_moe_att.py")] + stage2_args
    _run(cmd2, env)
