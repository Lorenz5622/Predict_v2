...
def import_moe_classes():
    ...
    for model_mod, cfg_mod in [
        ("qwen_moe.modeling.modeling_moe", "qwen_moe.modeling.configuration_moe"),
    ]:
        try:
            mod_model = __import__(model_mod, fromlist=["Qwen2MoeForCausalLM", "CrossAttentionRouter"])
            mod_cfg = __import__(cfg_mod, fromlist=["Qwen2MoeConfig"])
            return mod_model.Qwen2MoeForCausalLM, mod_cfg.Qwen2MoeConfig, mod_model.CrossAttentionRouter
        except Exception as exc:
            tried.append((model_mod, cfg_mod, repr(exc)))

    msg = "Failed to import Qwen2MoeForCausalLM/Qwen2MoeConfig. Tried:\n"
...
def _init_cross_attention_router_from_legacy_dense(
    ...
    with torch.no_grad():
        ...
        for layer_idx, layer in enumerate(model.model.layers):
            ...
            router = mlp.router
            dense_w = legacy_sd.get(f"model.layers.{layer_idx}.mlp.gate.weight")
            if dense_w is None:
...
def build_model_with_router_compat(
    ...
    is_kbit = quantization_config is not None
