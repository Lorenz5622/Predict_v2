#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Two-stage LoRA finetuning entrypoint for the MoEAtt token-level LM model."""
...
def import_moe_classes():
    ...
    for model_mod, cfg_mod in [
        ("modeling_moe_att_dm", "configuration_moe_att_dm"),
        ("qwen_moe.modeling.modeling_moe_att_dm", "qwen_moe.modeling.configuration_moe_att_dm"),
    ]:
        try:
...
def _init_cross_attention_router_from_legacy_dense(
    ...
    - expert_weight_proj: least-squares initialized to map expert-weight features
      toward the matching legacy dense router weight
    """
    num_inited = 0
    ...
    with torch.no_grad():
        for layer_idx, layer in enumerate(model.model.layers):
            ...
            if dense_w is None:
                if getattr(router, "expert_weight_proj", None) is not None:
                    per_layer_init_messages.append(
                        f"[init] layer {layer_idx} expert_weight_proj kept random init (legacy router.weight not found)"
                    )
                continue
            ...
            if hasattr(router, "initialize_expert_weight_projection_from_legacy_router"):
                if getattr(router, "expert_weight_proj", None) is not None:
                    router.initialize_expert_weight_projection_from_legacy_router(dense_w, mlp.experts)
                    per_layer_init_messages.append(
                        f"[init] layer {layer_idx} expert_weight_proj initialized from legacy router.weight"
                    )
...
def _forward_stage2_batch(
    ...
    *,
    ...
    expert_loss = getattr(out, "expert_loss", None)
    if expert_loss is None:
        expert_loss = ce_loss.new_zeros(())
    else:
        expert_loss = expert_loss.to(device=ce_loss.device, dtype=ce_loss.dtype)
    budget_loss = getattr(out, "router_budget_loss", None)
    if budget_loss is None:
...
def _warn_ignored_legacy_router_settings(args) -> None:
    ...
    if float(getattr(args, "router_value_anchor_loss_coef", 0.0)) > 0.0:
        ignored.append("router_value_anchor_loss_coef")
...
def train(
    ...
    current_router_top_k = int(max(1, getattr(run_args, "router_top_k", 2) or getattr(run_args, "router_topk", 2) or 2))
    ...
    if is_main_process():
        ...
        metrics_f.write("\t".join([
            ...
            "expert_embedding_pairwise_cos_mean", "expert_embedding_pairwise_cos_max",
            "expert_embedding_norm_mean", "expert_embedding_norm_std",
    ...
    for epoch in range(epochs):
        ...
        for step, batch in it:
            ...
            if global_step % grad_accum == 0:
                ...
                expert_weight_proj_grad_norm = _named_param_grad_norm(model, "router.expert_embedding_proj")
                ...
                if is_main_process() and metrics_f is not None and ((optim_step % log_every == 0) or should_eval):
                        metrics_f.write("\t".join([
                            ...
                            _metric_cell(router_forward_stats.get("expert_embedding_pairwise_cos_mean")),
                            _metric_cell(router_forward_stats.get("expert_embedding_pairwise_cos_max")),
                            _metric_cell(router_forward_stats.get("expert_embedding_norm_mean")),
                            _metric_cell(router_forward_stats.get("expert_embedding_norm_std")),
...
def configure_model_config(config, args) -> None:
    ...
    config.router_top_k = int(max(1, getattr(args, "router_top_k", 2) or getattr(args, "router_topk", 2) or 2))
    config.use_cross_attention_router = True
            ...
            for _step, batch in iterator:
                ...
                for layer_idx, mlp, hidden_states in captured:
                    ...
                    with torch.no_grad():
                        ...
                        expert_outputs = [
                            expert(flat_hidden).view(hidden_states.size(0), hidden_states.size(1), hidden_states.size(2))
                            for expert in mlp.experts
                        ]
                        expert_outputs = torch.stack(expert_outputs, dim=2)
                    student_logits, _, _ = mlp.router(hidden_states, expert_hidden_states=expert_outputs)
...
def parse_args():
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.0,
        help="Deprecated. Value-anchor loss was removed by dynamic expert-weight embeddings.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.0,
        help="Deprecated for MoEAtt. KL regularization is controlled by --moe_att_kl_loss_coef.",
    )
    ...
    ap.add_argument("--router_top_k", type=int, default=2)
    ap.add_argument("--router_topk", type=int, default=2)
    ap.add_argument(
        "--top_p_threshold",
        type=float,
        default=None,
    ...
    ap.add_argument("--init_new_router_from_legacy", type=int, default=0)
    ap.add_argument(
        "--router_use_entmax",
        type=int,
        default=0,
