...
def build_optimizer_4bit(model: nn.Module, lr: float, weight_decay: float, optim_name: str):
    ...
    if name == "adamw4bit":
        if not hasattr(bnb.optim, "AdamW4bit"):
            raise RuntimeError("bitsandbytes.optim.AdamW4bit is unavailable in current bitsandbytes version.")
        return bnb.optim.AdamW4bit(params, lr=lr, weight_decay=weight_decay)

    if name == "paged_adamw4bit":
        if not hasattr(bnb.optim, "PagedAdamW4bit"):
            raise RuntimeError("bitsandbytes.optim.PagedAdamW4bit is unavailable in current bitsandbytes version.")
...
def parse_args():
    ...
    ap.add_argument("--optim_4bit", type=str, default="paged_adamw4bit", choices=["adamw4bit", "paged_adamw4bit"])
