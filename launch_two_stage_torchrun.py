...
"""
Launcher for running stage1 then stage2 with two separate torchrun invocations.

Example:
  python launch_two_stage_torchrun.py --nproc_per_node 2 -- --config configs/finetune_dm.json
"""
...
def main() -> None:
    ...
    cmd1 = torchrun + torchrun_args + [str(repo_root / "run_stage1_dynamic_moe.py")] + stage1_args
    _run(cmd1, env)
    ...
    cmd2 = torchrun + torchrun_args + [str(repo_root / "run_stage2_dynamic_moe.py")] + stage2_args
    _run(cmd2, env)
